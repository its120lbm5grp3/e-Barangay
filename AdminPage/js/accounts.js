...
import {
	onAuthStateChanged, createUserWithEmailAndPassword, sendEmailVerification, signOut
}
from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js';
import {
	collection, getDocs, doc, setDoc, getDoc, updateDoc, serverTimestamp
}
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const createAccountButton = document.getElementById('create-account-btn') || document.querySelector('.form-section button') || document.querySelector('.btn-create');
	...
	async function loadAccounts() {
		...
		usersSnapshot.forEach(userDoc => {
			...
			if(createdAt && typeof createdAt.toDate === 'function') {
				dateCreated = createdAt.toDate().toLocaleDateString('en-US', {
					month: 'numeric',
					day: 'numeric',
					year: 'numeric'
				});
			...
			const unsubscribe = onValue(userStatusRef, (snapshot) => {
				...
				if(statusCell) {
					if(snapshot.exists() && snapshot.val().isOnline) {
						...
						const lastSeen = new Date(snapshot.val().last_changed).toLocaleString('en-US', {
							month: 'short',
							day: 'numeric',
							hour: '2-digit',
							minute: '2-digit'
